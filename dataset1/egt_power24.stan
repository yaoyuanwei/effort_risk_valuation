...
transformed parameters {
  ...
  for (i in 1:N) {
    ...
    lambda[i] = Phi_approx(mu_pr[6] + sigma[6] * lambda_pr[i]) * 5;
    beta[i]   = Phi_approx(mu_pr[7] + sigma[7] * beta_pr[i]) * 20;
  }
}
...
// Hyperbolic function
  ...
  for (i in 1:N) {
    ...
    for (t in 1:(Tsubj[i])) {
      c_effort  = k1[i] * pow(coste[i, t], p1[i]);
      c_risk    = k2[i] * pow(costr[i, t], p2[i]);
...
generated quantities {
  ...
  mu_lambda = Phi_approx(mu_pr[6]) * 5;
  mu_beta   = Phi_approx(mu_pr[7]) * 20;
