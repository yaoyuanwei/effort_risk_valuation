...
// Two-parameter power function
  ...
  for (i in 1:N) {
    ...
    for (t in 1:(Tsubj[i])) {
      // Cost of the gamble based on the power function
      c_gamble  = k[i]*pow(cost[i, t], p[i]);
...
generated quantities {
  ...
  mu_p      = Phi_approx(mu_pr[2]) * 5;
  mu_rho    = Phi_approx(mu_pr[3]);
  mu_lambda = Phi_approx(mu_pr[4]) * 5;
  mu_beta   = Phi_approx(mu_pr[5]) * 20;
