# hBayesDM Model Function tailored for two two-option cost-benefit decision-making tasks

hBayesDM_edt2 <- function(task_name,
                           model_name,
                           model_type = "",
                           data_columns,
                           parameters,
  ...
  function(data           = NULL,
    ...
    edt_preprocess_func <- function(raw_data, general_info) {
      ...
      cost_e      <- array( 0, c(n_subj, t_max))
      cost_r      <- array( 0, c(n_subj, t_max))
      amount_one  <- array( 0, c(n_subj, t_max))
      amount_two  <- array( 0, c(n_subj, t_max))
      ...
      for (i in 1:n_subj) {
        ...
        cost_e[i, 1:t]      <- DT_subj$coste
        cost_r[i, 1:t]      <- DT_subj$costr
        amount_one[i, 1:t]  <- DT_subj$amountone
        amount_two[i, 1:t]  <- DT_subj$amounttwo
    ...
    data_list <- edt_preprocess_func(raw_data, general_info)
