...
function [onsets, names, durations, nr_ind] = edt_extract_model(task_name, anal_fold, sub_data, run_n, sub_sv, si)
    ...
    for ri = 1:run_n
        ...
        % index for first and last trial of this run
        rtrial_head = (ri-1)*rtrial_n + 1;
        rtrial_end  = ri*rtrial_n;
        % trials of this run = 1
        ...
        % index for the position of the regressors in the contrast vector
        ...
        % using stick function, so duration = 0
    ...
    if max(nan_ind) > 0
        % note it in the no-response matrxi
        nr_ind(1,2) = 1;
        % get onset
